...
async def register_user(user: schemas.UserRegister, db: AsyncSession= Depends(get_db)):
    ...
    """Insert data now"""
    user_data= User(username=user.username, email=user.email, password=hashPassword)
    db.add(user_data)
    await db.commit()
...
async def login_user(user: OAuth2PasswordRequestForm= Depends(), db: AsyncSession= Depends(get_db)):
    ...
    result= await db.execute(stmt)
    existing_user = result.scalar_one_or_none()
    if not existing_user or not verify_password(existing_user.hashed_password, user.password):
